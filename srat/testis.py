...
def testis(prefix, library, collapser, devnull):
	...
	subprocess.call("bowtie -v 0 %s/testis2/index_piRNA -f %s --al %s --un %s > %s" % (library, unmapped_mir, map_piRNA, unmapped_piRNA, bowtie_piRNA_out), shell=True, stderr=devnull)
	# for other RNA
	unmapped_RNA = prefix + ".unmap_RNA.fa"
	bowtie_RNA_out = prefix + ".bowtie_RNA.out"
	subprocess.call("bowtie -v 0 %s/index_RNA --norc --suppress 6,7 -f %s --un %s > %s" % (library, unmapped_piRNA, unmapped_RNA, bowtie_RNA_out), shell=True, stderr=devnull)
	# for genome
