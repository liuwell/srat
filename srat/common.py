...
################
def common(prefix, pre_index, species, collapser, devnull):
	...
	subprocess.call("bowtie -v 0 %s/%s/index_RNA --norc --suppress 6,7 -f %s --un %s > %s" % (pre_index, species, unmapped_mir, unmapped_RNA, bowtie_RNA_out), shell=True, stderr=devnull)
	# for genome
...
def commonPlot(dic_length_RNA, total_RNA, prefix):
	...
	df_RNA = df.loc[:,['miRNA', 'tsRNA', 'rsRNA', 'snoRNA', 'misc_RNA', 'lincRNA', 'mRNA']]
